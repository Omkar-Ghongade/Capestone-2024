    ...
    return (
        <div className='shadow-md w-full fixed top-0 left-0 z-50 bg-[#272715]' style={{ fontFamily: 'Helvetica, Arial, sans-serif' }}>
            <div className='md:flex items-center justify-between h-16 md:px-10 px-7 m-0'>
                ...
                <ul className={`bg-[#272715] h-25 items-center md:h-10 md:flex md:items-center md:pb-0 pb-12 absolute md:static md:bg-[#272715] md:z-auto z-[-1] left-0 w-full md:w-auto md:pl-0 pl-9 transition-all duration-500 ease-in ${open ? 'top-12' : 'top-[-490px]'}`}>
                    {
                        navItems.map((link) => {
                                ...
                                return (
                                    <React.Fragment key={link.id}>
                                        {ProjectDropdown.map((item) => (
                                            <li onClick={() => setOpen(!open)} key={item.id} className='bg-[#272715] pt-1 md:ml-8 md:my-0 my-7 font-semibold'>
